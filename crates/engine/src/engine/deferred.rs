...
impl Engine {
    ...
    pub(crate) fn deferred_refs(
        ...
        // business.
        if accepts && let Some(r) = spec.config.values().find_map(reserved_later) {
            anyhow::bail!(
...
mod tests {
    ...
    /// reference — it is somebody else's syntax until it exists. `${src:0:3}` is
    /// bash substring expansion and `${env:FOO}` is a template syntax several
    /// tools use, and both are legal in a `run` today.
    ///
    ...
    fn a_reserved_but_unimplemented_kind_is_recognized_but_not_collected() {
        ...
        // …and the shell forms that share those names are untouched.
        assert!(reserved_later(&s("${src:0:3}")).is_some());
        assert!(reserved_later(&s("${FOO:-default}")).is_none());
    }
