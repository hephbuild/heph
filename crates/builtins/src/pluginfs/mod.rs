...
///
/// **Legacy.** The authority is the per-directory [`hwalk::codegen`] registry;
/// this remains only as a fallback for files stamped by an older binary and not
/// re-generated since. See [`codegen_owner`].
pub fn has_codegen_xattr(path: &std::path::Path) -> bool {
    matches!(xattr::get(path, CODEGEN_XATTR), Ok(Some(_)))
}

/// Who owns a generated tree file.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CodegenOwner<'a> {
    /// The registry names the owning `codegen = "copy"` target.
    Target(&'a str),
    /// No registry entry, but the file carries the legacy provenance xattr, so
    /// it is generated by *some* target this record cannot name.
    LegacyStamp,
}

/// The `codegen = "copy"` target that owns the tree file at `abs`, or `None` if
/// it is raw source.
///
/// `registry` is the [`hwalk::codegen::Registry`] of `abs`'s own directory and
/// `hashout` yields the file's current content hash (see
/// [`hwalk::file_hashout`]), computed only when there is a record to check it
/// against — a directory with no registry answers without touching the file.
///
/// The hash is what makes a stale record safe. A registry entry says "heph wrote
/// this name"; the hash says "and these are still the bytes it wrote". When they
/// disagree — a branch switch that checked a real source file in over a
/// generated path, a hand-edited generated file, another tool writing there —
/// the file is **not** heph's, and treating it as source is the only answer that
/// does not silently drop a real input from the build.
///
/// An unreadable file keeps its record: hiding a file heph cannot inspect leaves
/// a stale output out of the build, where revealing it would compile it.
fn codegen_owner_with<'a>(
    registry: &'a hwalk::codegen::Registry,
    abs: &std::path::Path,
    name: &str,
    hashout: impl FnOnce() -> Option<String>,
) -> Option<CodegenOwner<'a>> {
    if let Some(entry) = registry.get(name) {
        return match hashout() {
            Some(h) if entry.accepts(&h) => Some(CodegenOwner::Target(&entry.owner)),
            Some(_) => {
                tracing::warn!(
                    path = %abs.display(),
                    owner = %entry.owner,
                    "tree file is registered as generated but its content is not what heph \
                     wrote; treating it as source",
                );
                None
            }
            None => Some(CodegenOwner::Target(&entry.owner)),
        };
    }
    // Legacy: a file stamped by a binary that predates the registry, whose owner
    // has not run since. This costs a `getxattr` per unregistered file — i.e.
    // nearly every file — and goes away with the fallback, once a migration
    // sweep can convert existing stamps in one pass.
    has_codegen_xattr(abs).then_some(CodegenOwner::LegacyStamp)
}

/// [`codegen_owner_with`] over the shared walker, whose file hashes are cached
/// cross-run — the walk path, where the hash is wanted for the artifact anyway.
pub fn codegen_owner<'a>(
    registry: &'a hwalk::codegen::Registry,
    walker: &CachedWalker,
    abs: &std::path::Path,
    name: &str,
) -> Option<CodegenOwner<'a>> {
    codegen_owner_with(registry, abs, name, || {
        walker.file_hash(abs).ok().map(|fh| fh.hashout.clone())
    })
}

/// [`codegen_owner_with`] reading the file directly, for callers with no walker
/// (the engine's codegen write-back).
pub fn codegen_owner_uncached<'a>(
    registry: &'a hwalk::codegen::Registry,
    abs: &std::path::Path,
    name: &str,
) -> Option<CodegenOwner<'a>> {
    codegen_owner_with(registry, abs, name, || {
        let x = is_exec_path(abs);
        hwalk::file_hashout(abs, x).ok()
    })
}

/// The exec bit of the file at `path` (false when it cannot be stat'd), folded
/// into the content hash by [`hwalk::file_hashout`].
fn is_exec_path(path: &std::path::Path) -> bool {
    #[cfg(unix)]
    {
...
fn walk_glob(
    ...
    match std::fs::metadata(&walk_root) {
        ...
        // is no parent answer to carry: resolve it in full — and with no listing
        // to take the registry from, read the one file's own directory (a single
        // `open`, usually ENOENT).
        Ok(_) => {
            let registry = walk_root
                .parent()
                .map_or_else(hwalk::codegen::Registry::empty, |d| {
                    hwalk::codegen::Registry::load(d)
                });
            emit_glob_file(
                walker,
                root,
...
/// What a directory settles about every entry in it. Both facts are established
/// once per directory and carried down, rather than re-derived per file.
struct DirCtx<'a> {
    /// Whether the directory resolves inside a `.heph*` dir. See
    /// [`entry_resolves_into_heph_dir`].
    in_heph: bool,
    /// The directory's codegen provenance registry.
    registry: &'a hwalk::codegen::Registry,
}
...
fn emit_glob_file(
    ...
    if codegen_owner(dir.registry, walker, abs, name).is_some()
        || entry_resolves_into_heph_dir(abs, name, is_symlink, dir.in_heph)
    {
        return Ok(());
    }
    ...
    async fn run<'a, 'io>(
        ...
        match def {
            FsDef::File { path } => {
                ...
                // A file() over a net-new codegen output (registered in its
                // directory's `.hephgen`) resolves to nothing — the generated
                // content must only be sourced from its generator, never re-read
                // as raw source. A path resolving into a `.heph*` dir
                // (engine-internal artifact) is skipped the same way.
                //
                // The registry is read straight from the file's own directory
                // rather than through the walker: one `open` that usually
                // ENOENTs, against a `read_dir` of a directory this target may be
                // the only thing to touch.
                let fname = abs.file_name().and_then(|n| n.to_str()).unwrap_or_default();
                let registry = abs
                    .parent()
                    .map_or_else(hwalk::codegen::Registry::empty, |d| {
                        hwalk::codegen::Registry::load(d)
                    });
                if codegen_owner(&registry, &self.walker, &abs, fname).is_some()
                    || resolves_into_heph_dir(&abs)
                {
...
mod tests {
    ...
    /// Register `name` in `dir`'s `.hephgen` as generated by `owner`, recording
    /// the bytes it currently holds — what the write-back does, minus the engine.
    fn register_codegen(dir: &std::path::Path, name: &str, owner: &str) {
        let path = dir.join(name);
        let x = is_exec_path(&path);
        let mut reg = hwalk::codegen::Registry::empty();
        reg.upsert(hwalk::codegen::Entry {
            name: name.to_string(),
            owner: owner.to_string(),
            record: hwalk::codegen::Record::File {
                hash: hwalk::file_hashout(&path, x).unwrap(),
                prev: None,
            },
        });
        fs::write(dir.join(hwalk::codegen::REGISTRY_NAME), reg.render()).unwrap();
    }

    /// The registry file must never be sourced itself. Its name starts with
    /// `.heph`, which the walk already treats as engine-internal — so a glob
    /// that would otherwise match it (`**/*`) skips it without a special case.
    #[test]
    fn test_walk_glob_skips_the_registry_file_itself() {
        let tmp = tempdir().unwrap();
        let root = tmp.path();
        fs::write(root.join("a.rs"), b"aaa").unwrap();
        fs::write(
            root.join(hwalk::codegen::REGISTRY_NAME),
            b"heph-codegen 1\n",
        )
        .unwrap();

        let skip = Arc::new(Ignore::new(&[], &[]).unwrap());
        let compiled = compile_glob(&skip, "t", "**/*", &[]).unwrap();
        let walker = CachedWalker::disabled();

        let arts = walk_glob(&walker, root, &compiled).unwrap();
        let names: Vec<_> = arts.iter().map(|a| a.name.as_str()).collect();
        assert_eq!(names, vec!["a.rs"], "the registry is not workspace source");
    }

    /// The registry alone excludes a generated file — no xattr is set anywhere in
    /// this test, which is the point: the provenance has to survive every tool
    /// that keeps a file's bytes and drops its metadata.
    #[tokio::test]
    async fn test_driver_run_glob_excludes_registered_codegen() {
        let driver = Driver::default();
        let tmp = tempdir().unwrap();
        fs::write(tmp.path().join("plain.rs"), b"plain").unwrap();
        fs::write(tmp.path().join("generated.rs"), b"generated").unwrap();
        register_codegen(tmp.path(), "generated.rs", "//pkg:gen");
        ...
        let names: Vec<_> = res.artifacts.iter().map(|a| a.name.as_str()).collect();
        assert_eq!(names, vec!["plain.rs"], "registered file must be excluded");
    }

    /// A record is only as good as the bytes it names. Content heph did not write
    /// is source, however the path is registered — otherwise a branch switch that
    /// checks a real file in over a generated path would silently drop it from
    /// the build.
    #[tokio::test]
    async fn test_driver_run_glob_includes_a_registered_path_with_foreign_content() {
        let driver = Driver::default();
        let tmp = tempdir().unwrap();
        let generated = tmp.path().join("generated.rs");
        fs::write(&generated, b"generated").unwrap();
        register_codegen(tmp.path(), "generated.rs", "//pkg:gen");
        // Somebody else's bytes, same path, stale record.
        fs::write(&generated, b"hand written, not generated").unwrap();
        ...
        let names: Vec<_> = res.artifacts.iter().map(|a| a.name.as_str()).collect();
        assert_eq!(names, vec!["generated.rs"], "foreign content is source");
    }

    #[tokio::test]
    async fn test_driver_run_file_registered_codegen_yields_nothing() {
        let driver = Driver::default();
        let tmp = tempdir().unwrap();
        fs::write(tmp.path().join("generated.rs"), b"generated").unwrap();
        register_codegen(tmp.path(), "generated.rs", "//pkg:gen");
        ...
        assert!(
            res.artifacts.is_empty(),
            "file() over a registered codegen file must yield no artifacts"
        );
    }
