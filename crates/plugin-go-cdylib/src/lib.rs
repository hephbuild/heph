...
//!
//! Plugin-specific settings are read from the environment (as the spawned `bin:`
//! variant does); only the workspace root crosses in [`CreateConfig`].
...
use hplugin_stabby::abi::{CreateConfig, NamedDriver, PluginComponents};
use hplugin_stabby::serve_stable::{make_dyn_managed_driver, make_dyn_provider};
