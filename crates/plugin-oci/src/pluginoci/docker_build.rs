//! The `docker_build` driver: builds a container image archive from a Dockerfile
//! and a build context, with `docker buildx`, into a cacheable target output.
//!
//! Named for what it needs, not for what it makes: it requires a running docker
//! daemon and a buildx builder, it can `RUN` arbitrary commands, and it is not
//! fully hermetic — see "What is NOT an input" below for the list of things that
//! change the image without changing the cache key.
//!
...
//! `type=docker,dest=…` fail on it, and with them every `docker_build` target
...
//!
//! ```python
//! docker_build(name = "img", builder = "heph", ...)
//! ```
//!
...
use super::{
    ImageFormat, basename, dep_files, dep_single_file, is_addr, normalize_platform, ws_path,
};
...
pub const DRIVER_NAME: &str = "docker_build";

/// Builds a container image from a Dockerfile with `docker buildx`. Requires a
/// running docker daemon and a builder that can export to a file, and is not
/// fully hermetic (see `builder`, `secrets`, and the module docs).
#[derive(Spec)]
struct DockerBuildSpec {
    ...
    ///
    /// ```python
    /// docker_build(name = "img", dockerfile = ":dockerfile", context = [":srcs"])
    /// ```
    ///
    ...
    /// Base images, by build-context name → a single `oci_pull` (or `docker_build`)
    ...
    ///
    /// ```python
    /// docker_build(name = "img", builder = "multi",
    ///           platforms = ["linux/amd64", "linux/arm64"])
    /// ```
    ///
...
/// is empty.
pub(crate) const PLATFORM_ORIGIN: &str = "builder_platform";
...
impl Hash for DockerBuildDef {
    fn hash<H: Hasher>(&self, state: &mut H) {
        DOCKER_BUILD_FORMAT_VERSION.hash(state);
...
pub(crate) fn parse_builder_platform(inspect_out: &str) -> anyhow::Result<String> {
...
impl ManagedDriver for Driver {
    ...
    async fn parse(
        ...
        let spec =
            DockerBuildSpec::from(&req.target_spec.config).context("parse docker_build config")?;
        ...
        if platforms.is_empty() {
            let probe = super::platform::addr_for(builder.as_deref());
            inputs.push(Input {
        ...
        let hash = {
            let mut h = DebugHasher::new(Xxh3Default::new(), || {
                format!("docker_build_{}", addr.format())
            });
            def.hash(&mut h);
    ...
    async fn run<'a, 'io>(
        ...
        let def = req.request.target.def_de::<DockerBuildDef>().clone();
        ...
        let dockerfile_full = match &def.dockerfile {
            ...
            DockerfileSource::Dep => dep_single_file(&req, DOCKERFILE_ORIGIN)
                .context("docker_build: resolving the `dockerfile` target's output")?,
            DockerfileSource::Path(rel) => {
                let full = context_dir.join(rel);
                anyhow::ensure!(
                    full.exists(),
                    "docker_build: Dockerfile {rel:?} not found in the build context — declare the \
        ...
        let probed_platform = if def.platforms.is_empty() {
            let path = dep_single_file(&req, PLATFORM_ORIGIN)
                .context("docker_build: locating the builder-platform probe output")?;
...
fn build_error_hint(e: anyhow::Error, def: &DockerBuildDef, addr: &str) -> anyhow::Error {
    let text = format!("{e:#}");
    ...
    // daemon. `docker_build` always writes an archive, so on such a builder *every*
    ...
    if text.contains("exporter is not supported for the docker driver") {
        ...
        return e.context(format!(
            "docker_build {addr}: {fix} (or turn on the daemon's containerd image store, which gives \
             the `docker` driver the same exporters)"
        ));
    }

    if def.platforms.len() <= 1 {
        return e.context(format!("docker_build {addr}"));
    }
    ...
    e.context(format!("docker_build {addr}: {builder}{bases}"))
}
...
///   [`DockerBuildDef::builder_platform`] instead of directly: what matters for the
...
#[cfg(test)]
mod tests {
    use super::super::testfake::{Sandbox, run_request};
    use super::*;
...
"#;
    ...
    async fn parse_declares_context_inputs_and_two_outputs() {
        ...
        let def = resp.target_def.def::<DockerBuildDef>();
        assert_eq!(
            def.dockerfile,
    ...
    /// A base that is not a layout at all (a `docker_build` tar, say) must say so
    ...
    /// The plain `docker` driver has no file exporters, so a `docker_build` build
