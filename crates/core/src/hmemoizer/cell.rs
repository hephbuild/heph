...
// `mod tests` is last on purpose (`clippy::items_after_test_module`): production
// items below a test module read as test scaffolding and are easy to miss.
#[cfg(test)]
mod tests {
    use super::*;
    ...
    /// A cell under test, plus handles on the waker its inner future stashed and
    /// on the flag that makes that future ready.
    type StashCell = (Arc<Cell<u32>>, Arc<Mutex<Option<Waker>>>, Arc<AtomicBool>);

    fn stash_cell(value: u32) -> StashCell {
