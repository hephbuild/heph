...
impl Driver {
    ...
    async fn run_inner<'a, 'io>(
        ...
        // The builtins go directly behind the target's own tools: a recipe that
        // provisions its own `sed` gets that one, and everything it did not
        // declare comes from heph rather than from whatever this host ships.
        ...
        let path_policy = hexecrunner::PathPolicy {
            prefix: tool_bin_dir
                ...
                .into_iter()
                .chain(coreutils_dir.map(|d| d.as_os_str().to_os_string()))
                .collect(),
...
mod tests {
    ...
    #[tokio::test]
    async fn coreutils_sits_behind_the_targets_own_tools_on_path() -> anyhow::Result<()> {
        // The precedence the whole design rests on: a target that declares a
        // tool gets that one, and the builtins only displace the host's.
        let home = tempfile::tempdir()?;
        let driver = Driver::new_bash()
        ...
        assert_eq!(
            ...
            "the builtins must sit directly behind them; got: {path_out}"
        );
        assert!(
            path_out.contains("/usr/bin"),
            "what the target declared must still follow; got: {path_out}"
        );
        Ok(())
    }
}
