...
enum TestmainMode {
    ...
    Xtest,
    /// Both (legacy/combined). Kept for callers that haven't split yet. The
    /// default when `mode` is absent (back-compat with combined-bin callers).
    #[default]
    Both,
}

/// Config for a `go_testmain` target (engine-generated by the Go provider).
/// `#[derive(Spec)]` provides the parser and the LSP schema.
#[derive(Spec)]
struct GoTestmainSpec {
    /// Dependencies; `golist` (non-empty) plus optional `test`/`xtest` source groups.
    deps: HashMap<String, Vec<String>>,
    /// Which test binary to generate: `internal`, `xtest`, or `both` (default).
    mode: TestmainMode,
