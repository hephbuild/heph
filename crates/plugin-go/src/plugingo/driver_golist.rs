...
mod tests {
    ...
    /// targets, so the option is the *only* way this driver ever runs its tool
    /// somewhere other than the host — a driver that failed to consult its own
    /// default would leave the option silently inert.
    #[tokio::test]
    async fn the_driver_default_runner_reaches_the_parsed_inputs() {
        let ct = noop_ctoken();
        let req = make_parse_request("mylib", "example.com/mylib", vec![]);
        let resp = GoGolistDriver::new()
            .with_default_runner(Some("//tools/devenv:runner".to_string()))
            .parse(req, &ct)
            .await
            .expect("parse");
        let runner: Vec<_> = resp
            .target_def
            .inputs
            .iter()
