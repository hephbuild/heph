...
impl Engine {
    ...
    async fn upload_to_remote_inner(&self, addr: &Addr, hashin: &str) -> anyhow::Result<()> {
        ...
        let prepared: Vec<(String, PathBuf, ManifestArtifactEncoding)> = {
            ...
            heph_proc::process_supervisor::block_or_inline(move || -> anyhow::Result<_> {
                use std::io::Read;
    ...
    pub(crate) async fn download_from_remote(
        ...
        let res =
            heph_proc::process_supervisor::block_or_inline(move || -> anyhow::Result<Manifest> {
                use std::io::Write;
...
mod tests {
    ...
    fn addr() -> Addr {
        Addr::new(
            heph_model::htpkg::PkgBuf::from("p"),
            "t".to_string(),
            Default::default(),
        )
    }
