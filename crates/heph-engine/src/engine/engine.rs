...
pub struct PluginInit {
    ...
    pub walker: Arc<heph_walk::CachedWalker>,
}
...
pub struct Engine {
    ...
    pub(crate) walker: Arc<heph_walk::CachedWalker>,
    ...
    pub fn new(cfg: FuseConfig, home: &Path) -> anyhow::Result<Self> {
        ...
        match sandboxfuse::Mount::mount(&lower, fs.clone()) {
            Ok(m) => {
                ...
                heph_proc::process_supervisor::register_fuse_root(root.clone());
                Ok(Self {
                    root,
                    lower,
                    upper,
...
impl Engine {
    pub fn new(cfg: Config) -> anyhow::Result<Engine> {
        ...
        let walker = Arc::new(heph_walk::CachedWalker::open(
        ...
        engine.register_driver(|_| Box::new(heph_builtins::plugingroup::Driver))?;
        engine.register_provider(|_| Box::new(heph_plugin_query::pluginquery::Provider))?;
