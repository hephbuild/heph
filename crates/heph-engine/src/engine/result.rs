...
impl Engine {
    ...
    pub async fn result(
        ...
        while let Some(addr) = stream.try_next().await? {
            ...
            heph_core::hmemoizer::join_set_spawn(
                &mut set,
        ...
        crate::engine::event::emit_scope(
            ...
            async {
                ...
                let sizes: Vec<u64> = result
                    ...
                    .collect();
                heph_telemetry::telemetry::record_artifacts(result.artifacts.len() as u64, &sizes);
    ...
    async fn resolve_locked_inner(
        ...
        if !can_cache {
            ...
            let skip_lock = opts.spec.driver == heph_builtins::pluginfs::DRIVER_NAME;
            let _w = if skip_lock {
                None
            } else {
                Some(
    ...
    async fn execute_and_cache_inner(
        ...
        rs.data
            ...
            .once(
                key,
                enclose!((self => engine, rs) move || async move {
                    heph_core::hmemoizer::set_phase("execute_cache:engine_execute");
                    ...
                    heph_core::hmemoizer::set_phase("execute_cache:cache_locally");
                    ...
                    heph_core::hmemoizer::clear_phase();
                    out
                }),
            )
            .await
    ...
    async fn maybe_store_fixpoint(
        ...
        let dup = heph_proc::process_supervisor::block_or_inline(enclose!(
        ...
        let futures = inputs
            ...
            .map(|(i, input)| {
                ...
                enclose!((self => engine, rs) async move {
                    ...
                    let transitive = if spec.driver == heph_builtins::plugingroup::DRIVER_NAME {
...
fn rewrite_query_inputs(
    ...
    for input in &mut def.inputs {
        ...
        if r.package.as_str() == heph_plugin_query::pluginquery::PACKAGE {
...
mod tests {
    ...
    async fn query_recursive_includes_root_build_file() -> anyhow::Result<()> {
        ...
        engine.register_provider(|init| {
            Box::new(heph_plugin_buildfile::pluginbuildfile::Provider::new(
    ...
    async fn engine_wires_provider_functions_into_buildfile() -> anyhow::Result<()> {
        ...
        let mut srcs = match spec.spec.config.get("srcs") {
            Some(heph_core::htvalue::Value::List(l)) => l
                .iter()
                .map(|e| match e {
                    heph_core::htvalue::Value::String(s) => s.clone(),
    ...
    async fn get_spec_cross_target_cycle_returns_typed_error() -> anyhow::Result<()> {
        ...
        let addr_a = heph_model::htaddr::parse_addr("//pkg:a")?;
        let addr_b = heph_model::htaddr::parse_addr("//pkg:b")?;
        let rs = engine.new_state();
    ...
    async fn get_spec_falls_through_a_cyclic_provider_to_the_next() -> anyhow::Result<()> {
        ...
        let addr = heph_model::htaddr::parse_addr("//pkg:t")?;
    ...
    async fn get_spec_hard_fails_when_every_provider_cycles() -> anyhow::Result<()> {
        ...
        assert!(
            heph_core::hmemoizer::downcast_chain_ref::<CycleError>(&err).is_some(),
            "expected CycleError, got: {err:#}"
        );
        Ok(())
    }
    ...
    async fn query_input_annotated_with_origin_hash() -> anyhow::Result<()> {
        ...
        let input = def
            ...
            .inputs
            .iter()
            .find(|i| i.r#ref.r#ref.package.as_str() == heph_plugin_query::pluginquery::PACKAGE)
    ...
    async fn nested_fail_fast_off_records_aggregated_input_failures() -> anyhow::Result<()> {
        ...
        let addr = heph_model::htaddr::parse_addr("//pkg:parent")?;
        let rs = engine.new_state_with_fail_fast(false);
        let res = engine
            .clone()
            .result_addr(
    ...
    async fn diamond_failure_recorded_once_at_root() -> anyhow::Result<()> {
        ...
        let addr = heph_model::htaddr::parse_addr("//d:top")?;
        let rs = engine.new_state();
        let err = engine
            .clone()
            .result_addr(
            ...
            .spawn(|| {
                ...
                rt.block_on(async {
                    async fn run_chain(n: usize) -> (usize, usize, String) {
                        ...
                        let head = heph_model::htaddr::parse_addr(&addrs[0]).expect("addr");
                        let rs = engine.new_state();
                        let err = engine
                            .clone()
                            .result_addr(
    ...
    async fn classify_attaches_process_log_tail_to_recorded_failure() -> anyhow::Result<()> {
        ...
        let engine = engine_with(vec![static_target("//pkg:a", &[], &[])])?;
        let rs = engine.new_state();
        let addr = heph_model::htaddr::parse_addr("//pkg:a")?;
    ...
    async fn cache_hit_reads_manifest_once() {
        ...
        impl LocalCache for CountingCache {
            ...
            fn seekable_reader(
                ...
                hashin: &str,
                name: &str,
            ) -> anyhow::Result<Option<Box<dyn heph_core::hartifactcontent::ReadSeek + Send>>>
            {
    ...
    async fn in_place_allows_net_new_files() -> anyhow::Result<()> {
        ...
        let addr = heph_model::htaddr::parse_addr("//pkg:gen")?;
    ...
    async fn write_back_applies_exec_bit_on_unchanged_content() -> anyhow::Result<()> {
        ...
        let addr = heph_model::htaddr::parse_addr("//pkg:mkexec")?;
        let (res, _events) = resolve_collecting_events(&engine, &addr).await;
    ...
    async fn fixpoint_hit() -> anyhow::Result<()> {
        ...
        let addr = heph_model::htaddr::parse_addr("//pkg:fmt")?;
        let fmt = addr.format();
    ...
    async fn in_place_dep_is_not_written_back() -> anyhow::Result<()> {
        ...
        let consumer = heph_model::htaddr::parse_addr("//pkg:consumer")?;
        ...
        let fmt = heph_model::htaddr::parse_addr("//pkg:fmt")?;
    ...
    async fn stamped_copy_output_excluded_from_later_glob() -> anyhow::Result<()> {
        ...
        resolve_collecting_events(&engine, &heph_model::htaddr::parse_addr("//pkg:ip")?)
            .await
            .0
            .expect("in_place target resolves");

        // A glob over the stamped copy output must yield nothing.
        let gen_glob = heph_builtins::pluginfs::glob_addr("pkg/*.gen", &[]);
        ...
        let keep_glob = heph_builtins::pluginfs::glob_addr("pkg/keep.txt", &[]);
    ...
    async fn writeback_xattr() -> anyhow::Result<()> {
        ...
        if xattr_supported {
            assert!(
                xattr::get(&gen_file, heph_builtins::pluginfs::CODEGEN_XATTR)?.is_some(),
                "net-new copy output must carry the codegen xattr"
            );
            assert!(
                xattr::get(&src_file, heph_builtins::pluginfs::CODEGEN_XATTR)?.is_none(),
    ...
    async fn write_back_strips_exec_bit_on_unchanged_content() -> anyhow::Result<()> {
        ...
        let addr = heph_model::htaddr::parse_addr("//pkg:rmexec")?;
        let (res, _events) = resolve_collecting_events(&engine, &addr).await;
    ...
    async fn in_place_doubles_gc_history() -> anyhow::Result<()> {
        ...
        let fmt = Arc::clone(&engine)
            .get_def(rs.clone(), &heph_model::htaddr::parse_addr("//pkg:fmt")?)
            .await?;
        assert_eq!(
        ...
        let gen_def = Arc::clone(&engine)
            .get_def(rs.clone(), &heph_model::htaddr::parse_addr("//pkg:gen")?)
            .await?;
        assert_eq!(
