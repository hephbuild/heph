...
mod tests {
    ...
    fn exec_spec(addr: &str) -> TargetSpec {
        ...
        TargetSpec {
            addr: heph_model::htaddr::parse_addr(addr).expect("parse addr"),
    ...
    async fn introspect_outputs_expands_to_fs_inputs() -> anyhow::Result<()> {
        ...
        let want_glob = heph_builtins::pluginfs::glob_addr("pkg/*.go", &[]).format();
        assert!(
        ...
        let want_file = heph_builtins::pluginfs::file_addr("pkg/foo.txt").format();
        assert!(
