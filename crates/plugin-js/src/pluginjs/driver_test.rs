...
/// Dep groups staged read-only (a directory/file symlink, not a byte copy —
/// see `hdriver_support::stage`) rather than copied into the sandbox.
/// Mirrors plugin-go's own read-only annotation on its SDK/toolchain dep
/// group (`driver_compile.rs`), never on the package's own sources: none of
/// these three is ever written into by a running test. `"external"` is
/// resolved third-party/workspace-sibling code — npm packages don't get
/// mutated at test time. `"runner_config"`/`"tsconfig"` are config files a
/// test process only ever reads (js_test never shells out to `tsc`, so no
/// `--incremental`/`"composite"` `.tsbuildinfo` write into the tsconfig's own
/// directory can happen here — that's `js_typecheck`'s target, not this
/// one).
///
...
mod tests {
    ...
    /// (a symlink, not a byte copy — see `READ_ONLY_DEP_GROUPS`'s doc for
    /// why exactly these three); the test file's own first-party closure
    /// (`""`) and `"codegen"` must stay writable, or a Vitest snapshot write
    /// (or a co-located `js_src` codegen output) would hit EACCES.
    #[tokio::test]
    async fn parse_marks_only_external_runner_config_and_tsconfig_deps_read_only() {
        let ct = ctoken();
        let req = make_parse_request_with_deps(vec![
            ("", vec!["//@heph/fs:file@f=packages/a/src/index.test.ts"]),
            (
        ...
        let resp = driver().parse(req, &ct).await.unwrap();
        let read_only_groups: Vec<&str> = resp
            .target_def
            .inputs
            .iter()
            .filter(|i| {
        ...
        let writable_groups: Vec<&str> = resp
            .target_def
            .inputs
            .iter()
            .filter(|i| !i.annotations.contains_key(hdriver_support::stage::READ_ONLY_ANNOTATION))
            .map(|i| i.origin_id.split('|').nth(1).unwrap_or(""))
            .collect();
        assert_eq!(writable_groups, vec!["", "codegen"]);
    }
