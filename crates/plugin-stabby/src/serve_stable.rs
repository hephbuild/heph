...
use crate::abi::{DynExecutor, StableManagedDriver, StableProvider};
...
/// Wrap a real provider as an ABI-stable [`crate::abi::DynProvider`] handle
/// (in-process; the cdylib entry produces the same handle across the boundary).
pub fn make_dyn_provider(provider: Arc<dyn Provider>) -> crate::abi::DynProvider {
    stabby::boxed::Box::new(StableProviderImpl { provider }).into()
}

/// Wrap a real managed driver as an ABI-stable [`crate::abi::DynManagedDriver`].
pub fn make_dyn_managed_driver(driver: Arc<dyn ManagedDriver>) -> crate::abi::DynManagedDriver {
