...
func TestSuppressedGeneratedMarkerAfterPackage(t *testing.T) {
	// Some generators emit `package foo` first, then the generated comment (and
	// with a `!` instead of `.`). Lax must still detect it — the scan must not
	// stop at the package clause.
	dir := t.TempDir()
	file := filepath.Join(dir, "generated.go")
	src := "package infschemas\n\n// Code generated by github.com/x/gensonschema. DO NOT EDIT!\n\nvar X = 1\n"
	if err := os.WriteFile(file, []byte(src), 0o644); err != nil {
		t.Fatal(err)
	}
	s, err := newSuppressor(golangciConfig{})
	...
	if !s.suppressed("govet", file, 279, "literal copies lock value from c") {
		t.Error("generated marker below the package clause must be detected (lax)")
	}
}
...
// The suppressor is shared by every wrapped analyzer, and unitchecker runs them
// concurrently — so `suppressed` is called from many goroutines at once, and its
// two lazily-filled caches (nolint, genCache) are written on the miss path.
//
// This is not a "would be nice to synchronize" case: unsynchronized map writes
// abort the process with "fatal error: concurrent map writes", which no
// recover can catch, so a lint run dies outright. It went unseen while the
// default analyzer set was narrow; widening it to `go vet`'s full suite made
// two analyzers report at the same moment often enough to crash reliably.
//
// Many distinct files, so the miss path (the only writer) is hit hard. Run with
// -race for the direct signal; without it, the fatal map write is the symptom.
func TestSuppressorIsConcurrencySafe(t *testing.T) {
	dir := t.TempDir()
	const files = 24
	...
	for i := range paths {
		...
		src := "package p\n\nfunc F() {} //nolint:govet\n"
		if i%3 == 0 {
			src = "// Code generated by thing. DO NOT EDIT.\n" + src
		}
	...
	for g := range 16 {
		...
		go func() {
			defer wg.Done()
			for range 40 {
				// Walk the files from a different offset per goroutine so the
				// misses collide rather than lining up behind one warm entry.
				for i := range paths {
					p := paths[(i+g)%len(paths)]
					s.suppressed("govet", p, 3, "some message")
				}
			}
		}()
	}
