...
2. **Warmup run** — prime caches, FS, allocator. Discard output. `heph r test //...` must run from `example/`:
   ...
   ```bash
   cd $DEVENV_ROOT/example && samply record --unstable-presymbolicate -s "$(target-dir)"/profiling/heph r test //...
   ```
...
3. **Measured run** — same command, second invocation, plus pprof output:
   ...
   ```bash
   cd $DEVENV_ROOT/example && samply record --unstable-presymbolicate -s "$(target-dir)"/profiling/heph --pprof-cpu=/tmp/cpu.prof r test //...
   ```
