...
func Untar(in io.Reader, to string, o UntarOptions) (err error) {
	...
	if o.Progress != nil {
		...
		go func() {
			progressChan := progress.NewTicker(ctx, pin, 0, 10*time.Millisecond)
...
func UntarList(ctx context.Context, in io.ReadCloser, listPath string, progresss func(read int64)) ([]string, error) {
	...
	err := Walk(xprogress.NewReaderTap(in, u), func(hdr *tar.Header, tr *tar.Reader) error {
		switch hdr.Typeflag {
