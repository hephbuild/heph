...
pub struct TargetSpec {
    pub addr: Addr,
    pub driver: String,
    pub config: HashMap<String, Arc<dyn Any + Send + Sync>>,
...
pub trait Provider: Send + Sync {
    fn config(&self, req: ConfigRequest, ctoken: &(dyn Cancellable + Send + Sync)) -> anyhow::Result<ConfigResponse>;
...
impl Provider for StaticProvider {
    ...
    fn list<'a>(&'a self, _req: ListRequest, _ctoken: &'a (dyn Cancellable + Send + Sync)) -> BoxFuture<'a, anyhow::Result<Box<dyn Iterator<Item=ListResponse>>>> {
        Box::pin(async move {
            let items: Vec<_> = self.targets.iter().map(|t| ListResponse{addr: t.addr.clone()}).collect();
