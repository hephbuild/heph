...
func TestSanityRemoteCache(t *testing.T) {
	...
	for i := 1; i <= 2; i++ {
		...
		{
			res, err := e.Result(ctx, pkg, "t1", []string{""}, engine.ResultOptions{}, &engine.ResolveCache{})
			require.NoError(t, err)
		...
		{
			res, err := e.Result(ctx, pkg, "t2", []string{""}, engine.ResultOptions{}, &engine.ResolveCache{})
			require.NoError(t, err)
			defer res.Unlock(ctx)
		...
		{
			res, err := e.Result(ctx, pkg, "t3", []string{""}, engine.ResultOptions{}, &engine.ResolveCache{})
			require.NoError(t, err)
			defer res.Unlock(ctx)
