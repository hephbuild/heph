...
func TestDepsCacheLarge(t *testing.T) {
	ctx := t.Context()
	...
	staticprovider := pluginstaticprovider.New([]pluginstaticprovider.Target{
		{
			Spec: pluginv1.TargetSpec_builder{
				...
				Driver: htypes.Ptr("bash"),
				Config: map[string]*structpb.Value{
					"run": hstructpb.NewStringsValue([]string{`dd if=/dev/zero of=$OUT bs=1k count=200`}),
					"out": hstructpb.NewStringsValue([]string{"out_child"}),
				},
			}.Build(),
		},
		{
			Spec: pluginv1.TargetSpec_builder{
				...
				Driver: htypes.Ptr("bash"),
				Config: map[string]*structpb.Value{
					"run":  hstructpb.NewStringsValue([]string{`wc -c < "${SRC}" | tr -d ' ' > $OUT`}),
					"deps": hstructpb.NewStringsValue([]string{"//:child"}),
					"out":  hstructpb.NewStringsValue([]string{"out_parent"}),
				},
			}.Build(),
...
func TestDepsCacheRemote(t *testing.T) {
	...
	for _, key := range []string{"__child/16f2c56460420e9a/manifest.v1.json", "__child/16f2c56460420e9a/out_out.cpio"} {
		cache.EXPECT().
			...
			DoAndReturn(func(ctx context.Context, key string, reader io.Reader) error {
				...
				cache.EXPECT().
					Get(gomock.Any(), key).
					Return(io.NopCloser(bytes.NewReader(b)), nil).AnyTimes()
