...
- **Per-OS behavior is the user's decision.** Features work uniformly on Linux and macOS by default — those two are the whole supported set (CI builds `x86_64-unknown-linux-gnu`, `aarch64-unknown-linux-gnu`, `aarch64-apple-darwin`); no BSD, no Windows. A divergence between Linux and macOS may be the right answer — but it is never settled by an agent or by the implementation. Flag it (what differs, on which OS, what each option costs) and put the call to the user before writing the code.
